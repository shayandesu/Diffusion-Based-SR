...
class BPD(NLL):
    def compute(self) -> Tensor:
        ...
        Returns:
            bpd
        """
        return super().compute() / LOG2  # Use the parent's compute() which gives the mean
...
class Perplexity(NLL):
    def compute(self) -> Tensor:
        ...
        Returns:
            Perplexity
        """
        return torch.exp(super().compute())  # Use parent's compute() for the mean
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    # self.smiles_tokenizer.load_vocabulary('/home/xulei/shayan/SR/smiles-mdlm/smiles_vocab.txt')
  ...
  def on_validation_epoch_start(self):
    ...
    # nll_mean = self.valid_metrics.nll.compute()
    
    # assert nll_mean == 0 or torch.isnan(nll_mean).item(), f"Expected nll to be 0, got {nll_mean}"
